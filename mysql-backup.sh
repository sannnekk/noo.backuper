...
mysqldump -u$DB_USER -p$DB_PASSWORD --lock-tables=false --no-tablespaces --max_allowed_packet=5G --quick --single-transaction --master-data=1 -h $DB_HOST $DB_NAME | gzip > /var/backup/backup-$(date +%Y-%m-%d-%H-%M-%S).sql.gz
